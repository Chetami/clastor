...
final class ClastorUITests: XCTestCase {
    ...
    func testSignInAndSignOut() {
        ...
        XCTAssertEqual(app.tabBars.buttons.allElementsBoundByIndex.map(\.label),
                       ["Home", "Students", "Calendar", "Payments", "Profile"])
        app.tabBars.buttons["Students"].tap()
        XCTAssertTrue(app.staticTexts["Alex Example"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["Sam Example"].exists)
